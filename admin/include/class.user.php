<?php
    include "db_config.php";
        class user
        {
            ...
            public function add_room($roomname, $room_qnty, $no_bed, $bedtype,$facility,$price)
            {
                    ...
                    $sql="INSERT INTO room_category SET roomname='$roomname', available='$available', booked='$booked', room_qnty='$room_qnty', no_bed='$no_bed', bedtype='$bedtype', fac='$facility', price='$price'";
                    $result= mysqli_query($this->db,$sql) or die(mysqli_connect_errno()."Data cannot inserted");
                
                
                    for($i=0;$i<$room_qnty;$i++)
                    {
                        $sql2="INSERT INTO rooms SET roomname='$roomname', book='false'";
            ...
            public function check_available($checkin, $checkout)
            {
                   ...
                   $sql="SELECT DISTINCT roomname FROM rooms WHERE room_id NOT IN (SELECT DISTINCT room_id
   FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin') )";
            ...
            public function booknow($checkin, $checkout, $name, $phone,$roomname)
            {
                    ...
                    $sql="SELECT * FROM rooms WHERE roomname='$roomname' AND (room_id NOT IN (SELECT DISTINCT room_id
   FROM rooms WHERE checkin <= '$checkin' AND checkout >='$checkout'))";
             ...
             public function edit_room_cat($roomname, $room_qnty, $no_bed, $bedtype,$facility,$price,$room_cat)
            {
                        ...
                        $sql2="DELETE FROM rooms WHERE roomname='$roomname'";
                        mysqli_query($this->db,$sql2);
                 
                 
                        for($i=0;$i<$room_qnty;$i++)
                        {
                            $sql3="INSERT INTO rooms SET roomname='$roomname', book='false'";
                        ...
                        $sql="UPDATE room_category  SET roomname='$roomname', available='$available', booked='$booked++', room_qnty='$room_qnty', no_bed='$no_bed', bedtype='$bedtype', fac='$facility', price='$price' WHERE roomname='$room_cat'";
